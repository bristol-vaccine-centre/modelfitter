...
format_summary = function(
  ...
  if (attributes(summfit)$fits_retained) {
    ...
    grp_structure = summfit %>%
      ...
      dplyr::mutate(
        labels = purrr::imap(
          model_fit,
          ~ attr(.x, "labels") %>% dplyr::mutate(model.order = .y)
        )
      ) %>%
      dplyr::reframe(
  ...
  if (global.p) {
    method = lapply(table_data$fmt, dplyr::pull, global.p.method) %>%
      purrr::list_c() %>%
      unique()
    footer_text = c(
  ...
  character_order = unique(unlist(purrr::map(
    table_data$fmt,
    ~ .x %>% dplyr::pull(characteristic)
  )))
  subgroup_order = unique(unlist(purrr::map(
    table_data$fmt,
    ~ .x %>% dplyr::pull(subgroup)
  )))
  ...
  tmpy = table_data2 %>%
    ...
    tidyr::complete(
      col_head,
      tidyr::nesting(characteristic, subgroup),
      fill = list(statistic = "\u2014", p_value = "\u2014")
    ) %>%
    dplyr::mutate(
  ...
  unique = all(
    (tmpy %>%
      dplyr::group_by(col_head, characteristic, subgroup) %>%
      dplyr::count() %>%
      dplyr::pull(n)) ==
      1
  )
...
`{model_type_name}` may help."
  ...
  chr_name = getOption(
    "modelfitter.characteristic_column_name",
    "Characteristic"
  )
  ...
  rgv = if (global.p) {
    sapply(c(chr_name, pvalue_name, subgrp_name), as.symbol)
  } else {
    sapply(c(chr_name, subgrp_name), as.symbol)
  }
  ...
  out = table_data2 %>%
    ...
    dplyr::mutate(
      hux = purrr::map2(col_head, fmt, function(.x, .y) {
        # browser()
  ...
  init = init %>%
    huxtable::set_colspan(row = 1, col = seq(1, ncol(init) - 2, 2) + 2, 2)

  return(init %>% .hux_set_footer(footer_text))
}
...
format_model = function(model_name, fit, statistic, ...) {
  ...
  model_fit = tibble::tibble(
    ...
    performance = list(suppressMessages(performance::model_performance(
      fit,
      verbose = FALSE
    ))),
    fit = list(fit)
  ...
  tmp2 = structure(tmp, fits_retained = TRUE, performance = TRUE)
